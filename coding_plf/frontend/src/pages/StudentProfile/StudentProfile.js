...
import { FaEdit, FaSignOutAlt, FaKey, FaEnvelope, FaLink, FaGraduationCap, FaUserMinus, FaPlus, FaTrash } from 'react-icons/fa';
import { useNavigate } from 'react-router-dom';

const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'https://coding-club-1.onrender.com';
const PROFILE_URL = process.env.NEXT_PUBLIC_PROFILE_ENDPOINT || `${API_BASE_URL}/api/profile`;
...
const LOCAL_KEY = 'studentProfile';
...
const StudentProfile = () => {
  ...
  const addSkill = () => setEditProfileData({ ...editProfileData, skills: [...editProfileData.skills, ''] });
  const removeSkill = (index) => setEditProfileData({ ...editProfileData, skills: editProfileData.skills.filter((_, i) => i !== index) });
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100">
      ...
      {loading ? (
        ...
        <div className="max-w-5xl mx-auto p-6 space-y-6">
            ...
            >
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <input name="address" value={editProfileData.address} onChange={handleInputChange} placeholder="Address" className="w-full p-2 rounded bg-gray-700"/>
                ...
                <div className="space-y-2">
                  {editProfileData.skills.map((skill, idx)=>(
                    <div key={idx} className="flex items-center gap-2">
                      <input value={skill} onChange={(e)=>handleSkillChange(idx,e.target.value)} className="p-2 rounded bg-gray-700 flex-1"/>
                      <button onClick={()=>removeSkill(idx)} className="bg-red-600 px-2 py-1 rounded"><FaTrash/></button>
                    </div>
                  ))}
                  <button onClick={addSkill} className="flex items-center bg-green-600 px-3 py-1 rounded mt-2"><FaPlus className="mr-1"/> Add Skill</button>
                </div>
          ...
          {/* Skills Section */}
          <div className="bg-gray-800 rounded-xl p-6">
            <h2 className="text-2xl font-semibold border-b border-gray-700 pb-2 mb-2">Skills</h2>
            {profileData.skills.length ? (
              <div className="flex flex-wrap gap-2">
                {profileData.skills.map((skill, idx) => (
                  <span key={idx} className="bg-indigo-700 px-3 py-1 rounded-full text-sm">{skill}</span>
                ))}
              </div>
          ...
          {/* Education */}
          <div className="bg-gray-800 rounded-xl p-6">
            <h2 className="text-2xl font-semibold border-b border-gray-700 pb-2 mb-2 flex items-center"><FaGraduationCap className="mr-2"/> Education</h2>
          ...
          {/* Contact & Social */}
          <div className="bg-gray-800 rounded-xl p-6">
            <h2 className="text-2xl font-semibold border-b border-gray-700 pb-2 mb-2"><FaLink className="mr-2"/> Contact & Social</h2>
            <p>Phone: {profileData.phone || 'Not added'}</p>
            <p>Address: {profileData.address || 'Not added'}</p>
